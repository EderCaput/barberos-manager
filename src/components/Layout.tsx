  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-sidebar border-r border-sidebar-border shadow-lg z-20">
        ...
        <nav className="flex-1 px-4 py-6 space-y-2 relative overflow-y-auto overflow-x-hidden">
          {navItems.map((item) => {
        ...
        <div className="p-4 border-t border-sidebar-border bg-sidebar/50 backdrop-blur-md space-y-3">
          <button
            onClick={signOut}
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex flex-col items-center gap-1.5 px-3 text-[10px] font-medium transition-all ${active ? 'text-primary scale-110' : 'text-muted-foreground hover:text-foreground'
                  }`}
              >
